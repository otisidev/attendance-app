...
const resolvers = {
    Query: {
        ...
        GetDepartmentalStudentForEnrollment: async (_, __, { dataSources, user }) => {
            if (user) {
                const { department: d, level } = __;
                const { _studService } = dataSources;
                return await _studService.GetStudentsByDepartmentForEnroll(d, level);
            }
            return new AuthenticationError("Unauthorized Access!");
        },
        ...
        GetCoursemate: async (_, __, { dataSources, user }) => {
            if (user) {
                ...
                const departmentalCourseId = student_res.doc.registeredCourses.map(i => i.departmentalCourse);
                const result = await _studService.GetStudentsOfSameClass(departmentalCourseId, session_res.doc.id);
                return result;
            }
            return new AuthenticationError("Unauthorized Access!");
        },
        GetRegisteredCourses: async (_, __, { dataSources, user }) => {
            if (user) {
                ...
                // session
                const session_res = await _sService.GetActiveSession();
                const result = await _studService.GetStudentRegisteredCourseList(user.id, session_res.doc.id);
                return result;
            }
            return new AuthenticationError("Unauthorized Access!");
        }
    ...
    Mutation: {
        ...
        UpdateStudent: async (_, __, { dataSources, user }) => {
            if (user) {
                const { name, phone } = __;
                return await dataSources._studService.UpdateStudent(user.id, name, phone);
            }
            return new AuthenticationError("Unauthorized Access!");
        },
        UpdateStudentBiometric: async (_, __, { dataSources, user }) => {
            ...
            if (user) {
                ...
                const result = await _studService.UpdateFingerprint(student, template);
        ...
        UpdateSingleStudentBiometric: async (_, __, { dataSources, user }) => {
            ...
            if (user) {
                ...
                const { _studService } = dataSources;
                const result = await _studService.UpdateFingerprint(id, template);
                return result;
            }
            return new AuthenticationError("Unauthorized Access!");
        },
        ...
        StudentLogin: async (_, { no, password }, { dataSources }) => {
            ...
            // match the password
            const match = await coreService.ComparePasswords(password, student_res.doc.passphase);
            // password validation
        ...
        UpdateStudentLevel: async (_, { level }, { dataSources, user }) => {
            if (user) {
                return await dataSources._studService.UpdateLevel(user.id, level);
            }
            return new AuthenticationError("Unauthorized Access!");
        },
        UpdateStudentImage: async (_, { image_path }, { dataSources, user }) => {
            if (user) {
                return await dataSources._studService.UpdateStudentImage(user.id, image_path);
            }
            return new AuthenticationError("Unauthorized Access!");
        },
        StudentCourseRegistration: async (_, __, { dataSources, user }) => {
            if (user) {
                ...
                // Course registration
                const result = await _studService.UpdateRegisteredCourse(user.id, courses, session_res.doc.id, level);
                return result;
            }
            return new AuthenticationError("Unauthorized Access!");
        },
        UploadStudentBiometricData: async (_, { file }, { dataSources, user }) => {
            if (user) {
                ...
                return new ApolloError("File not properly formatted! It must be a list containing id and fingerprint props", "404");
            }
            return new AuthenticationError("Unauthorized Access!");
        }
    },
    Student: {
        ...
        registered_courses: async ({ registeredCourses }, _, { dataSources }) => {
            if (registeredCourses.length) return await dataSources.loaders.dcLoader.loadMany(registeredCourses.map(x => x.departmentalCourse.toString()));
            return [];
        },
